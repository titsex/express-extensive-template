...
export class TokenService {
    ...
    public static validateRefreshToken(token: string): CreateDto | null {
        try {
            return verify(token, process.env.JWT_REFRESH_KEY!) as CreateDto
        } catch (error) {
            return null
        }
    }

    public static validateAccessToken(token: string): CreateDto | null {
        try {
            return verify(token, process.env.JWT_ACCESS_KEY!) as CreateDto
        } catch (error) {
            return null
        }
    }
